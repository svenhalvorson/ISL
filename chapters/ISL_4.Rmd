...
a. Investigate the data set for patterns
...
b. Here's a logistic model using everything except `Year` & `Today`:
...
c. The confusion matrix looks like this:
```{r 10_3, echo=TRUE}
...
d. Set the data prior to 2009 as the training set and the rest of the data as the test set. Use only lag2 as a predictor and report the confusion matrix. Repeat this for LDA, QDA, and KNN with K=1. I'll skip the code but here are the results:
...
form = as.formula('Direction ~ Lag2')

weekly_logistic2 = glm(
  form,
  data = weekly_train,
  family = 'binomial'
)
...
# Logistic model
weekly_test$logistic = predict.glm(
  weekly_logistic2,
  newdata = weekly_test,
  type = 'response'
)
weekly_test$logistic = ifelse(
  weekly_test$logistic > 0.5,
  'Up',
  'Down'
) %>% 
...
# KNN with K=1
weekly_test$knn = knn(
  train = dplyr::select(weekly_train, Lag2),
  test = dplyr::select(weekly_test, Lag2),
...
weekly_test %>% 
  summarize(
    across(
      logistic:knn,
      .fns = list(
        'accuracy' = function(x){mean(x == weekly_test$Direction)},
        'fp' = function(x){sum(x == 'Up' & weekly_test$Direction == 'Down')},
        'fn' = function(x){sum(x == 'Down' & weekly_test$Direction == 'Up')}
      )
    )
  ) %>% 
  pivot_longer(
  ...
  separate(dum, into = c('model', 'measure')) %>% 
  pivot_wider(
    names_from = measure,
    values_from = val
  )
...
```{r 10_5_2, echo=FALSE}
# Write a function to do all the models with the specified predictors
make_models = function(pred_list){
  
  form =  paste(
    'Direction ~',
    paste0(pred_list, collapse = ' + ')
  ) %>% 
    as.formula()
  ...
  # KNN with K=1
  knn1_p = knn(
    train = dplyr::select(weekly_train, one_of(pred_list)),
    test = dplyr::select(weekly_test, one_of(pred_list)),
    cl = weekly_train[['Direction']],
    k = 1
  )
  ...
  summarize(
    across(
      everything(),
      .fns = list(
        'accuracy' = function(x){round(mean(x == weekly_test$Direction),3)},
        'fp' = function(x){sum(x == 'Up' & weekly_test$Direction == 'Down')},
        'fn' = function(x){sum(x == 'Down' & weekly_test$Direction == 'Up')}
      )
    )
  ...
  mutate(
    preds = paste0(pred_list, collapse = ' + ')
  )
}
...
# copy pasta
make_models2 = function(pred_list, nm){
  ...
  # KNN with K=1
  knn1_p = knn(
    train = dplyr::select(auto_train, one_of(pred_list)),
    test = dplyr::select(auto_test, one_of(pred_list)),
    cl = auto_train[['mpg01']],
    k = 1
  )
  ...
  summarize(
    across(
      everything(),
      .fns = list(
        'accuracy' = function(x){round(mean(x == auto_test$mpg01),3)},
        'fp' = function(x){sum(x == 1 & auto_test$mpg01 == 0)},
        'fn' = function(x){sum(x == 1 & auto_test$mpg01 == 1)}
      )
    )
  ) %>% 
  pivot_longer(
