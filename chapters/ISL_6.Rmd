...
The final step is to create regression models using these predictors. Typically, we would use a cross validation technique to determine when to stop adding components. Note that we would not end up with models that involve skipping any of the components as they should be decreasing in utility. 
...
3. This is not a subset selection method. Typically each of the components will use every predcitor so the total number needed is the same as in the original data set.
